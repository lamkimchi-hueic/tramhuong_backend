 ...
 * Called when server starts to ensure site_settings table has default data
 */
...
async function initSiteSettings(prisma) {
  if (!prisma) {
    throw new Error('Prisma client is required for initSiteSettings');
  }
  ...
  try {
    ...
    if (count === 0) {
      console.log('📝 Site settings table is empty. Creating default settings...');
      ...
      console.log(`✅ Created ${DEFAULT_SETTINGS.length} default site settings`);
    } else {
      console.log(`✓ Site settings already initialized (${count} settings found)`);
    }
  } catch (error) {
    console.error('❌ Error initializing site settings:', error.message);
  }
  // NOTE: Do NOT disconnect prisma here - it's used by the app
}

module.exports = { initSiteSettings };
